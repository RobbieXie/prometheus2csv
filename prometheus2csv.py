...
TiandiQueryList = [
    'sum by (name) (rate(container_cpu_usage_seconds_total{image!="",name="%s",container_label_org_label_schema_group=""}[1m]))  * 100',
...
def main():
    ...
    csvset = query_tiandi_metric_values(metricnames=metricnames)
...
def handle_args(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "h:o:c:s:",
                                   ["host=", "outfile=", "container=", "step=", "help", "start=", "end=", "period="])
    ...
    if PROMETHEUS_URL == '':
        ...
        logging.error(
            "You should use -c or --container to specify the name of the container which you want to query all the metrics of")
        print_help_info()
        sys.exit(2)
...
def query_tiandi_metric_names():
    pass
    return TiandiMetricNames

def query_tiandi_metric_values(metricnames):
    csvset = dict()
    ...
    for index in range(len(TiandiQueryList)):
        response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
                                params={'query': TiandiQueryList[index] % CONTAINER, 'start': start_time,
...
def query_metric_names():
    response = requests.get(PROMETHEUS_URL + QUERY_API,
                            params={'query': 'sum by(__name__)({{name="{0}"}})'.format(CONTAINER)}, auth=('admin', 'admin'))
...
def query_metric_values(metricnames):
    ...
    response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
                            params={'query': '{0}{{name="{1}"}}'.format(metric,CONTAINER), 'start': start_time,
                                    'end': end_time, 'step': RESOLUTION}, auth=('admin', 'admin'))
    status = response.json()['status']
